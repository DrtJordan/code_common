...
package org.code.flume.plugin.sink;
...
public class KafkaSink extends AbstractSink implements Configurable {
  ...
  public void configure(Context context) {
    ...
    useAvroEventFormat = context.getBoolean(AVRO_EVENT,
                                            DEFAULT_AVRO_EVENT);

    partitionHeader = context.getString(PARTITION_HEADER_NAME);
    staticPartitionId = context.getInteger(STATIC_PARTITION_CONF);

    if (logger.isDebugEnabled()) {
      logger.debug(AVRO_EVENT + " set to: {}", useAvroEventFormat);
    }
